...
function parseReview($review: CheerioSelection, $: CheerioAPI): Review | null {
  try {
    ...
    // 주의: .atc_cont 전체를 사용하면 .tit(요약)과 .atc(본문)가 합쳐져 중복됨
    const $atcExp = $review.find('.atc_exp');
    const $atcInCont = $review.find('.atc_cont .atc');  // .atc_cont 내의 .atc만 선택
    const $atcDirect = $review.find('.atc, .rvw_atc');  // 직접 .atc 선택
