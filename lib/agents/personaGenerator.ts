...
🚨 **배제 조건 감지 및 처리 (CRITICAL)**:
사용자가 다음과 같이 **명시적으로 특정 소재/기능/특성을 배제**하는 경우:
- "유리 배제해주세요", "유리는 안 돼요", "유리 제품 빼고"
- "스테인리스 배제", "스테인리스 말고", "스테인리스 제외"
- "플라스틱 없는 거", "플라스틱 제외"
- "쿨링팬 없는 것", "자동 기능 필요 없음"

👉 **반드시 다음 형식으로 contextualNeeds 최상단에 추가**:
- "❌ [소재/기능명] 제품 절대 제외"
- 예시: "❌ 유리 소재 제품 절대 제외", "❌ 스테인리스 소재 제품 절대 제외"

⚠️ 배제 조건은 **가장 강력한 제약**이므로 contextualNeeds 리스트의 **첫 번째 항목**으로 배치하세요.

🔄 **모순 조건 처리 (CRITICAL)**:
**추가 요청사항**에서 기존 배제/선호 조건과 **정반대**되는 내용이 있는 경우:
- 예: 기존 대화에 "유리 재질이 아닌 안전한 소재 선호"가 있는데, 추가 요청에 "유리 재질이였으면 좋겠어요"
- 예: 기존에 "❌ 스테인리스 소재 제외"가 있는데, 추가 요청에 "스테인리스로 해주세요"
...
⚠️ 모순이 발견되면 **최신 요청(추가 요청사항)을 반영**하고, 상충되는 기존 조건은 포함하지 마세요.
