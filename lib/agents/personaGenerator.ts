...
# 입력
사용자와의 전체 대화 내역 (추가 요청사항 + 구조화된 질문 + 자유 대화)
⚠️ 대화 시작 부분의 "사용자의 추가 요청사항:"이 있다면 이를 **최우선적으로** contextualNeeds에 반영하세요!
...
- **추가 요청사항에서 언급된 구체적 상황을 최우선**으로 추출
- 짧고 명확한 키워드 형태 (예: "쌍둥이 육아 중", "외출 많음", "새벽 수유 빈번")
- 3-7개 정도로 핵심만 추출
- 제품 추천 이유 작성에 직접 활용되므로 **매우 구체적**으로 작성

예시:
- 입력: "쌍둥이라 동시에 분유를 자주 타요"
- contextualNeeds: ["쌍둥이 육아 중", "동시 분유 준비 필요", "빠른 조유 필요"]
