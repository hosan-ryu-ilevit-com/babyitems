...
# 평가 지침
1. **🚨 배제 조건 최우선 확인 (CRITICAL)**:
   - contextualNeeds에서 "❌ [항목] 절대 제외" 형식의 배제 조건 확인
   - **배제 조건에 해당하는 제품은 무조건 overallScore 1점으로 평가**
   - 배제 조건 예시:
     • "❌ 유리 소재 제품 절대 제외" → 유리 포트는 overallScore 1점
     • "❌ 스테인리스 소재 제품 절대 제외" → 스테인리스 포트는 overallScore 1점
     • "❌ 플라스틱 제품 절대 제외" → 플라스틱 부품 많은 제품은 overallScore 1점
   - **배제 조건 위반 시 다른 모든 장점과 무관하게 낮은 점수 부여**

2. **세부 평가 항목 체크**: 각 속성의 세부 평가 항목들을 제품 상세 분석과 대조하여 얼마나 충족하는지 확인해주세요
3. **상세 분석 우선 참고**: 제품의 coreValues 점수는 참고용이며, 실제 평가는 상세 분석의 장단점을 기반으로 수행해주세요
4. **가중치 고려**: 사용자가 높은 가중치(9-10)를 준 속성에 집중해주세요
5. **맥락 반영**: contextualNeeds를 고려한 실질적 평가를 해주세요
6. **구체적 이유**: 상세 분석에서 언급된 구체적 특징과 세부 평가 항목을 이유에 포함해주세요
7. **사용자 관점**: 페르소나의 상황(예: 쌍둥이, 여행 빈도)과 제품의 실제 특성(상세 분석 참고)을 연결해주세요
8. **맥락적 적합도**: 숫자 점수가 높아도 페르소나의 contextualNeeds에 맞지 않으면 낮은 grade를 부여해주세요
9. **전체 점수**: 페르소나의 핵심 니즈(가중치 9-10)를 얼마나 충족하는지가 overallScore의 핵심이에요

**평가 예시:**
- **배제 조건 위반**: contextualNeeds에 "❌ 유리 소재 제품 절대 제외"가 있고 제품이 유리 포트인 경우 → overallScore 1점, 각 속성 "매우 미흡" 또는 "미흡"
