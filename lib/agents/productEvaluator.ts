...
**이 상세 분석은 실제 구매자 리뷰를 바탕으로 작성된 내용입니다:**
- **장점**: 실제로 검증된 강점
- **단점**: 실제 사용자들이 경험한 문제점
...
# 평가 지침
1. **세부 평가 항목 체크**: 각 속성의 세부 평가 항목들을 제품 상세 분석과 대조하여 얼마나 충족하는지 확인
2. **상세 분석 우선 참고**: 제품의 coreValues 점수는 참고용이며, 실제 평가는 상세 분석의 장단점을 기반으로 수행
3. **가중치 고려**: 사용자가 높은 가중치(9-10)를 준 속성에 집중
4. **맥락 반영**: contextualNeeds를 고려한 실질적 평가
5. **구체적 이유**: 상세 분석에서 언급된 구체적 특징과 세부 평가 항목을 이유에 포함
6. **사용자 관점**: 페르소나의 상황(예: 쌍둥이, 여행 빈도)과 제품의 실제 특성(상세 분석 참고)을 연결
7. **맥락적 적합도**: 숫자 점수가 높아도 페르소나의 contextualNeeds에 맞지 않으면 낮은 grade 부여
8. **전체 점수**: 페르소나의 핵심 니즈(가중치 9-10)를 얼마나 충족하는지가 overallScore의 핵심
