...
⚠️ **[스타일 주의] 나이/성별을 억지로 문장에 끼워 넣지 마세요.**
- ❌ 잘못된 예: "20개월 남아에게 적합한 디자인은?"
- ✅ 올바른 예: "어떤 디자인 스타일을 선호하시나요?"
- **원칙**: 이미 수집한 정보는 질문문에 반복 노출하지 말고, 내부 추천 로직에 활용하세요.
- **예외**: 아직 수집되지 않았고 안전/착용 적합성에 필수인 경우에는, 별도의 독립 질문으로 자연스럽게 수집하세요.
`;
    }
  }
...
8. **브랜드 질문 생성 금지:**
   - 맞춤질문 단계에서는 브랜드 선호/제외 질문을 절대 생성하지 마세요.
   - 브랜드 관련 질문은 최종 추천 로딩 단계의 별도 UI에서 수집됩니다.
   - 따라서 질문 id에 brand/brand_preference를 사용하지 마세요.

9. **아기용품 추가 컨텍스트 질문 (조건부):**
   - 아래는 **아기용품 카테고리에서만**, 그리고 **해당 정보가 실제 구매결정에 중요한 경우에만** 적용합니다. (무조건 생성 금지)
   - 적용 대상 예시: 카시트, 기저귀, 아기띠/힙시트, 보행기, 유아 의류/신발, 체형/착용 적합성이 중요한 용품
   - 불필요 대상 예시: 단순 소모품/가전 등 아기 신체 정보와 직접 관련이 낮은 품목
   - 정보가 비어 있다면 다음 질문을 우선순위 높게 고려:
     * 월령/나이 (착용/사용 가능 시기 판단에 필요할 때)
     * 몸무게/체중대 (권장 체중 구간이 중요한 카테고리일 때)
     * 성별 (기저귀/의류 등 성별 옵션이 실제로 존재할 때만)
   - **과도 수집 금지:** 위 항목을 한 번에 다 묻지 말고, 카테고리에 필요한 1개부터 우선 질문
   - **월령 질문 옵션 규칙 (중요):**
    * 월령/나이 질문을 생성한다면 옵션에 반드시 "출산 이전 (임신 중/준비 중)" 항목을 포함하세요.
    * 예시 옵션: "출산 이전 (임신 중/준비 중)", "0~3개월 (신생아)", "4~6개월", "7~12개월", "13개월 이상"
...
1. **⭐ 중복 방지 - 최우선 규칙 (가장 중요!):**
  ...
  - **Target Audience Check:**
    * "${categoryName}"이 아기용품이고 월령 정보가 구매결정에 중요하며 아직 수집되지 않았다면, 초반 질문으로 월령/나이 질문을 고려하세요.
    * 월령 질문을 생성할 경우 옵션에 반드시 "출산 이전 (임신 중/준비 중)"을 포함하세요.
    * 몸무게/성별도 카테고리 적합성에 실제로 필요할 때만 질문하세요. (기저귀/카시트/착용형 용품 등)
    * ⚠️ **[사용자가 이미 입력한 정보]에 해당 값이 있다면 같은 질문을 다시 생성하지 마세요!**
   - **Preference Check:**
