...
evidence는 비교표에서 충족도 아이콘 아래에 표시됩니다.
**1문장으로 핵심만 압축하세요. 사용자 조건을 반복하지 마세요.**

### 작성 원칙
1. **반드시 1문장** - 핵심 특성 + 구체적 근거를 한 문장에 압축
2. **제품 관점**으로 작성 - "~해요" 형식
3. **구체적 근거 포함** - 스펙 수치, 리뷰 인용, 기술명 등
4. 근거가 없으면 절대 추측하지 말고, "확인 필요" 문장 사용
5. **핵심 키워드 강조** - evidence 안의 핵심 키워드나 리뷰 인용 1~2개는 **굵게** 감싸기 (예: **저소음 설계**, **613,480원**)

### ✅ Good Examples (반드시 이 형식으로!)
- "**IH 압력 방식**으로 균일하게 가열되어 리뷰에서 **밥맛이 좋다**는 평가가 많아요."
- "**에코 스테인리스 내솥**이라 코팅 벗겨짐 없이 **오래 사용**할 수 있어요."
- "**10인용 대용량**이라 대가족도 한번에 충분히 지을 수 있어요."
- "**저소음 설계**로 리뷰에서 **밤에도 아기가 안 깬다**는 평가가 많아요."
...
- 판단: 61만원 < 77만원 → "full" ✅
- evidence: "제품 가격 613,480원으로 **희망 예산 77만원 이하**에 충분히 여유 있게 들어와요."
...
function buildFallbackProsCons(reviews: ReviewLite[]): {
  ...
  keywordPros.forEach((kw) => {
    pushUnique(pros, `${kw}이 좋아요`);
  });
  mentions.forEach((m) => {
    pushUnique(pros, `${m}이 좋아요`);
  });
  if (pros.length < 3 && qualitative.avgRating >= 4) {
    pushUnique(pros, '만족도가 높아요');
  }
  if (pros.length < 3 && qualitative.sentimentScore > 0.1) {
    pushUnique(pros, '호평이 많아요');
  }
  if (pros.length < 3) {
    pushUnique(pros, '사용후기가 있어요');
  }

  keywordCons.forEach((kw) => {
    pushUnique(cons, `${kw}이 아쉬워요`);
  });
  if (cons.length < 2 && qualitative.sentimentScore < -0.1) {
    pushUnique(cons, '호불호가 있어요');
  }
  if (cons.length < 2) {
    pushUnique(cons, '개인차가 있어요');
  }
  mentions.forEach((m) => {
    if (cons.length < 2) {
      pushUnique(cons, `${m} 의견이 나뉘어요`);
    }
  });
  if (cons.length < 2) {
    pushUnique(cons, '환경에 따라 달라요');
  }
...
### 장점 (pros) - 3~4가지
- 리뷰에서 실제로 언급된 핵심 장점을 **한줄 리뷰 태그** 형태로 작성
- 구체적이고 자연스러운 표현 사용 (3~10글자)
- 리뷰에 근거가 있는 것만 작성 (억지로 채우지 말 것)
- 예: "착용감이 편해요", "신축성이 좋아요", "통풍이 잘 돼요", "가성비 좋아요", "세척이 편해요"

### 단점 (cons) - 1~2가지
- 리뷰에서 실제로 언급된 아쉬운 점을 **한줄 리뷰 태그** 형태로 작성
- 리뷰에 근거가 없으면 0~1개만 작성해도 됨 (억지로 채우지 말 것)
- 예: "주머니가 없어요", "비침이 있어요", "소음이 있어요", "무거워요", "건조가 느려요"
...
⚠️ 리뷰에 언급 없는 내용은 작성 금지 — 근거 없으면 개수를 줄일 것
⚠️ 태그만 출력 (부가 설명 붙이지 말 것)`;
