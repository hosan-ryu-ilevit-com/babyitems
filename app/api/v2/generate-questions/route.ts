...
⚠️ **가장 중요한 원칙: 진짜 트레이드오프만 질문하세요!**

트레이드오프란? **A를 얻으면 반드시 B를 포기해야 하는 물리적/구조적 상반 관계**

✅ **진짜 트레이드오프 (이런 질문만 생성!)**
- "가벼움 vs 내구성" → 가벼운 소재는 구조적으로 약함
- "넓은 용량 vs 휴대성" → 크면 들고 다니기 불편
- "빠른 온도 조절 vs 보온력" → 열 전도 좋으면 보온 약함
- "분리 세척 vs 누수 방지" → 부품 많으면 틈새 생김

❌ **가짜 트레이드오프 (절대 생성 금지!)**
- "모유실감 좋음 vs 배앓이 방지" → 둘 다 가능! 상반 관계 아님
- "안전성 vs 편의성" → 둘 다 좋은 제품 많음
- "디자인 vs 기능성" → 예쁘면서 기능 좋은 제품 존재
- "A브랜드 vs B브랜드" → 브랜드는 트레이드오프 아님

**자가 검증 필수**: 질문 생성 전 스스로 물어보세요:
"이 두 특성을 동시에 가진 제품이 후보군에 있는가?"
→ YES면 트레이드오프 아님! 다른 질문 찾으세요.

**priority 타입은 최후의 수단**
- 진짜 트레이드오프를 못 찾았을 때만 사용
- "둘 다 좋긴 한데 뭐가 더 중요?" 형태
- 사용자가 "둘 다요"라고 느끼면 실패한 질문

형식 요구사항:
- type: "tradeoff" (기본) 또는 "priority" (최후 수단)
- title: 상반 관계가 명확히 드러나는 제목 (예: "가벼움 vs 튼튼함")
- option_A.text: **포기해야 하는 것이 암시된 구체적 문장** (30~50자)
  예시: "무겁더라도 튼튼하고 오래 쓸 수 있는 게 좋아요"
  예시: "좀 약하더라도 가볍고 들고 다니기 편한 게 좋아요"
- option_B.text: **반대 선택의 대가가 암시된 구체적 문장** (30~50자)
