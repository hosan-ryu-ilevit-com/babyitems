...
**⚠️ 중요: 제공된 리뷰는 샘플링된 일부입니다. 전체 리뷰 개수나 비율(%)을 언급하지 마세요!**

**반드시 다음을 수행하세요:**
1. 샘플링된 리뷰에서 사용자가 선택한 조건과 관련된 키워드가 몇 개 발견되었는지 카운트
2. 긍정 리뷰 몇 개, 부정 리뷰 몇 개인지 절대적 개수만 언급
3. 리뷰 원문을 짧게 인용하되, "리뷰 ~개에서", "~개 발견" 같은 절대적 표현만 사용
4. "X개 중 Y개", "X%가 긍정적" 같은 비율 표현은 절대 금지
...
5. **실제 사용자 리뷰를 필수로 참고하여 스펙과 실사용 경험이 일치하는지 확인**
   ...
   - 리뷰 카운트: 샘플링된 리뷰에서 특정 장점/단점이 몇 개 발견되었는지 절대적 개수만 카운트
  ...
  "top3": [
    {
      ...
      "rank": 1,
      "recommendationReason": "✅ 순수 한글로만 작성 (1~3문장)",
      "matchedPreferences": [
        "✅ 선택하신 빠른 가열 (리뷰 8개에서 언급)",
        "✅ 세척 편의성 (긍정 리뷰 7개 발견)",
        "✅ 저소음 (부정 리뷰 단 2개)"
      ]
    },
...
- **절대적 개수만 언급**: "리뷰가 많아요" (❌) → "긍정 리뷰 8개에서 언급" (✅)
- **스펙 → 실사용 효과 연결**: "300W 고출력" (❌) → "300W로 2분 내 데워져 새벽 수유가 빨라요" (✅)
- **부정 리뷰도 개수로**: "소음 적음" (❌) → "소음 불만 리뷰 단 2개" (✅)
...
- **⚠️ 절대 금지**: "X개 중 Y개", "X%", "대부분", "많은" 같은 비율/전체 표현
- **⚠️ 영어 절대 금지**: 영어 알파벳, 언더스코어(_) 사용 금지

※ recommendationReason은 1~2문장, 최대 3문장으로 작성하세요.
※ **작성 후 반드시 확인**: 응답에 영어가 있으면 안 됩니다!

### 좋은 예시 (절대적 개수 + 스펙-효과 연결 + 리뷰 개수)
- "선택하신 빠른 가열 조건에 맞춰, 300W 고출력으로 2분 내 데워져요. 실제로 긍정 리뷰 8개에서 '새벽 수유 때 빨리 먹여서 좋다'고 언급했어요"
- "세척 편의를 중요하게 보셨는데, 분리형 3단 구조로 구석구석 닦을 수 있어요. '세척 편하다'는 리뷰 7개 발견했고, 평점 4.7/5.0이에요"
- "소음을 걱정하셨죠. 저소음 모터(35dB)라 조용하고, 소음 불만 리뷰는 단 2개뿐이에요"
- "가볍고 휴대 편한 걸 원하셨는데, 850g으로 한 손에 들고 다닐 수 있어요. 여행용으로 쓴다는 리뷰 6개 발견했어요"
- "신생아부터 써야 한다고 하셨죠. 0개월부터 36개월까지 쓸 수 있고, '신생아 때부터 잘 썼다'는 리뷰 8개 확인했어요"
- "스테인리스 소재를 선호하셨는데, 304 스테인리스로 내구성이 좋아요. 1년 이상 사용자 리뷰 7개 모두 '변색/파손 없다'고 해요"

### 나쁜 예시 (절대 사용하지 마세요) - 영어 레이블 특히 주의!
**🚫 영어 레이블 노출 (절대 금지!):**
...
- ❌ "85개 리뷰 중 68개가 긍정적" (샘플링된 리뷰인데 전체 비율 언급)
- ❌ "대부분의 리뷰가" / "많은 사용자가" (비율 표현 금지)

### 작성 원칙 (우선순위 순)
1. **🚨 영어 0%**: 응답에 영어 알파벳, 언더스코어(_) 절대 금지 (pcode 제외)
2. **절대적 개수만 언급**: 리뷰 개수, 평점, 스펙 수치 (비율/전체 표현 금지)
3. **사용자 선택 조건 명시적 연결**: "선택하신 [조건]에 맞춰", "[선택한 특성]을 중요하게 보셨는데"
4. **스펙 → 실사용 효과 체인**: 스펙 수치 → 그로 인한 사용자 경험 → 리뷰 근거
5. **긍정+부정 리뷰 모두 활용**: "긍정 리뷰 Y개" 또는 "단점 언급 리뷰 단 Y개"
6. **이 제품만의 차별점**: 다른 제품과 구분되는 구체적 특징 + 수치
7. **자연스러운 대화체**: 정량적이되, 딱딱하지 않게 한국어로 자연스럽게
8. **리뷰 인용 간결하게**: 리뷰 원문은 짧게 따옴표로, 전체 맥락은 자연스럽게

---

⚠️ **최종 확인 (필수!)**
응답을 작성한 후, recommendationReason과 matchedPreferences를 다시 읽어보세요.
영어 단어(dishwasher, lightweight, rule, hf 등)나 언더스코어(_), 비율 표현(X개 중 Y개, X%)이 하나라도 있으면,
**반드시 자연스러운 한글로 다시 작성**하세요.

변환 예시:
- ❌ "dishwasher_yes" → ✅ "식기세척기 사용 가능"
- ❌ "lightweight 특성" → ✅ "가벼운 무게"
- ❌ "85개 중 68개" → ✅ "긍정 리뷰 8개에서 언급"
