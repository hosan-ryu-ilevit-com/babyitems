...
## 규칙
1. 선택지에 있는 어려운 용어(예: PPSU, 모유실감, 절충형 등)가 있다면 그것을 간단히 설명해주세요.
2. 선택지 중 무엇을 선택해야 하는지, 어떤 것이 인기인지 등은 말하지 마세요.
3. 질문과 무관한 정보(예: 화소 질문인데 렌즈커버 언급, 브랜드 선택 질문인데 '가벼운 제품 찾아보세요' 등)은 절대 금지입니다.
4. 1문장 이내로 작성하세요.
5. 친근한 반말 어투 (예: "~예요", "~이에요")
6. 이모티콘 금지, JSON 형식 금지, 순수 텍스트만 반환
...
- "좋은 제품을 고르세요" (의미 없음)`;
    ...
    const tip = await callGeminiWithRetry(async () => {
