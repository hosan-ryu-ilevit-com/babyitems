...
6. **selectionReasons:** 각 선택에 대한 개별 이유를 **한글로, 1문장으로** 작성하세요.
   ...
   **주의:** 사용자가 직접 언급하지 않은 것을 "원하신다", "선호하신다"로 표현하지 마세요. AI가 추론한 것은 "~~가 중요할 것 같아요", "~~도 체크했어요"로 표현하세요.
7. **overallReasoning:** 전체 선택에 대한 3-4문장 요약.
   **중요 - 사용자 직접 입력 vs AI 자동 선택 구분:**
   - 사용자가 직접 언급한 것 (userContext, clarifyingAnswers에 명시됨): "~~를 원하시는 점", "~~를 중요하게 생각하시는 점" 표현 사용
   - AI가 상황 추론으로 자동 선택한 것 (기본조건, 상세선호, 피할단점에서 AI가 판단): "~~도 함께 고려했어요", "~~도 중요할 것 같아 체크했어요" 표현 사용
   예시 (좋음): "**6개월 아기**에게 맞는 제품을 찾으시는군요. **가벼운 것**을 원하시는 점 반영했고, 안전 인증이나 세척 편의성도 함께 고려했어요."
   예시 (나쁨 - 하지 마세요): "안전한 소재를 선호하시는 점도 반영하여~" (사용자가 언급 안 했는데 선호한다고 표현)
   **핵심 키워드는 **키워드** 형식으로 감싸서 강조해주세요.**
